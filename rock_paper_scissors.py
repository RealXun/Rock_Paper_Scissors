...
while 1:
    first_input = int(input("Maximum number of games? (minimum3): ")) #The maximun numbers of rounds we are gonna play should be decided
    if is_odd(first_input) and is_more_or_equal_3(first_input): # Here we tell it umber is odd and Number is equal or greater than 3, correct, lets play
        ...
        while played < max_rounds:
            ...
            import random
            random = random.randrange(0, 3) #randomly returns one of the 3 options for the machines
            print("1)rock","\n2)paper","\n3)scissors")
            if random == 0:
                machine = "rock"
            elif random == 1:
        ...
        if users_win<machine_win or machine_win==to_win:
            ...
            print("So sad, There are no winners, TIE!!!!")
        sys.exit() #I have put it so that the infinite loop ends when reaching the number of games that were wanted to be played
    else:
        if not is_odd(first_input) or is_more_or_equal_3(first_input): # Here we tell it number isnot odd and Number is not equal or greater than 3, so incorrect. Pick again
